...
from triplet_reid import common, loss, lbtoolbox as lb
...
def sample_k_fids_for_pid(pid, all_fids, all_pids, batch_k):
    ...
    padded_count = (
        tf.cast(tf.ceil(batch_k / tf.cast(count, tf.float32)), tf.int32) * count
    )
...
def train(
    ...
    **kwargs,
    ...
    crop_augment = kwargs.get("crop_augment", False)
    flip_augment = kwargs.get("flip_augment", False)
    resume = kwargs.get("resume", False)
    loss_type = kwargs.get("loss", "batch_hard")
    metric = kwargs.get("metric", "euclidean")
    model_name = kwargs.get("model_name", "resnet_v1_50")
    head_name = kwargs.get("head_name", "fc1024")
    embedding_dim = kwargs.get("embedding_dim", 128)
    initial_checkpoint = kwargs.get("initial_checkpoint", None)
    margin = kwargs.get("margin", "soft")
    checkpoint_frequency = kwargs.get("checkpoint_frequency", 1000)

    pre_crop_height = kwargs.get("pre_crop_height", 288)
    pre_crop_width = kwargs.get("pre_crop_width", 144)
    loading_threads = kwargs.get("loading_threads", 8)

    learning_rate = kwargs.get("learning_rate", 0.0003)
    train_iterations = kwargs.get("train_iterations", 25000)
    decay_start_iteration = kwargs.get("decay_start_iteration", 15000)
    detailed_logs = kwargs.get("detailed_logs", False)

    current_time_and_date = str(datetime.now())[:-7]
    experiment_name_with_time = current_time_and_date + "_" + experiment_name
    experiment_root = os.path.join(evaluation, experiment_name_with_time)
    ...
    if resume:
        ...
        with open(args_file, "w") as f:
            json.dump(kwargs, f, ensure_ascii=False, indent=2, sort_keys=True)

    log_file = os.path.join(experiment_root, "train")
    ...
    train_set = os.path.join(evaluation, train_csv)
    image_root = os.path.join(evaluation, "0/model_outputs")
    if not train_set:
        parser.print_help()
    ...
    pids, fids = common.load_dataset(train_set, image_root)
    ...
    dataset = dataset.map(
        lambda pid: sample_k_fids_for_pid(
    ...
    dataset = dataset.map(
        lambda fid, pid: common.fid_to_image(
            fid, pid, image_root=image_root, image_size=net_input_size
        ),
    ...
    losses, train_top1, prec_at_k, _, neg_dists, pos_dists = loss.LOSS_CHOICES[loss_type](
    ...
    tf.summary.scalar("batch_prec_at_{}".format(batch_k - 1), prec_at_k)
    ...
    if detailed_logs:
        log_embs = lb.create_or_resize_dat(
            os.path.join(experiment_root, "embeddings"),
            dtype=np.float32,
            shape=(train_iterations, batch_size, embedding_dim),
        )
        log_loss = lb.create_or_resize_dat(
            os.path.join(experiment_root, "losses"),
            dtype=np.float32,
            shape=(train_iterations, batch_size),
        )
    ...
    if 0 <= decay_start_iteration < train_iterations:
        learning_rate = tf.train.exponential_decay(
            ...
            train_iterations - decay_start_iteration,
            0.001,
        )
    else:
    ...
    with tf.Session() as sess:
        if resume:
            ...
            last_checkpoint = tf.train.latest_checkpoint(experiment_root)
            ...
            checkpoint_saver.save(
                sess, os.path.join(experiment_root, "checkpoint"), global_step=0
            )

        merged_summary = tf.summary.merge_all()
        summary_writer = tf.summary.FileWriter(experiment_root, sess.graph)
        ...
        with lb.Uninterrupt(sigs=[SIGINT, SIGTERM], verbose=True) as u:
            for i in range(start_step, train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids = sess.run(
                    [
                        train_op,
                        merged_summary,
                        global_step,
                ...
                seconds_todo = (train_iterations - step) * elapsed_time
                log.info(
                ...
                if (
                    ...
                    and step % checkpoint_frequency == 0
                ):
                    checkpoint_saver.save(
                        sess,
                        os.path.join(experiment_root, "checkpoint"),
                        global_step=step,
                    )
        ...
        checkpoint_saver.save(
            sess, os.path.join(experiment_root, "checkpoint"), global_step=step
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--loss",
        default="batch_hard",
        choices=loss.LOSS_CHOICES.keys(),
    ...
    train(experiment_name="test",
          evaluation="/export/home/rmarwaha/projects/logs/2019-09-23T16:25:15_prjoti_lin_motion_visual/eval/2019-10-08T17-06-57_reid_test",
          train_csv="frame_anchor_1.csv",
          **args.__dict__)
